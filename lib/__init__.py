...
CURSOR = CONN.cursor() #CURSOR is a constant that allows us to interact with our database. The cursor() method creates a Cursor object. Cursor objects are are bound to the connection for the entire lifetime and all the commands are executed in the context of the database session wrapped by the connection.
